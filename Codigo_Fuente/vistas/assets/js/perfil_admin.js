...
var formularioPerfil = document.getElementById('perfilAdmin');
...
formularioPerfil.addEventListener('submit', function (e) {
  ...
  fetch('../../../ajax/perfil_admin.ajax.php', {
    method: 'POST',
    body: datos
  })
    .then(res => res.json())
    .then(data => {
      ...
      if (data == 'incompletos') {
        ...
        if (data == 'Actualizada') {
          document.getElementById("perfilAdmin").reset();
          Swal.fire({
            icon: 'success',
            title: 'Se ha actualizado',
            text: 'El administrador actualizó con exito',
          })
        } else {
          if (data == 'contraseñas') {
